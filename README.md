...
ticket, posts the generated plan back as an issue comment, then swaps the
issue's label from `forge:triage` to `forge:ready`. The label swap is the last
step, so any earlier failure leaves the issue in `forge:triage` for the next
pass to retry.
...
directory, and posts the agent's raw output back to the public issue with no
human review. A crafted issue can therefore attempt prompt injection to read
local files (secrets, private source) and have them echoed into the public
comment.
...
- Add a human-in-the-loop review of the generated plan before it is posted, if
  the target issues are public.
